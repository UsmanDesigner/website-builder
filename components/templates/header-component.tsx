...
import { headerTemplates } from "@/lib/component-templates"
...
export function HeaderComponent({ component, styles }: HeaderComponentProps) {
  const template = headerTemplates.find((t) => t.id === component.template)

  if (!template) {
    return <div>Header template not found</div>
  }
  ...
  // Modify the content to include URLs for menu items
  const contentWithUrls = {
    ...component.content,
    menuItemUrls: component.content.menuItemUrls || [],
  }
  ...
  return (
    <div style={containerStyle} className={combinedStyles.width !== "full" ? "max-w-screen-xl mx-auto" : ""}>
      {backgroundElement}
      <div style={{ position: "relative", zIndex: 2 }}>{template.render(contentWithUrls, combinedStyles)}</div>
    </div>
  )
}
